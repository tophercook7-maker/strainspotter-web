...
interface RegularFreePaywallProps {
  isOpen: boolean;
  onClose: () => void;
  onUpgrade: () => void;
}

export default function RegularFreePaywall({ isOpen, onClose, onUpgrade }: RegularFreePaywallProps) {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-md w-full mx-4">
        <h2 className="text-2xl font-bold text-emerald-300 mb-4">No Regular Scans Remaining</h2>
        
        <p className="text-slate-400 mb-6">
          You've used all your free scans this month. Upgrade to continue scanning.
        </p>

...
import { parseStoredScanResult } from "@/lib/scanner/savedScanMappers";
...
export async function fetchScansHistoryForUser(
  ...
  try {
    ...
    let q = supabase
      .from("scans")
      .select("id, primary_name, confidence, created_at, result")
    ...
    return data.map((row: Record<string, unknown>) => {
      ...
      return {
        id: String(row.id),
        primary_name: (row.primary_name as string) ?? null,
...
export async function fetchUnifiedScanByServerId(
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("scans")
      .select("id, result, primary_name, created_at")
      .eq("id", id)
      .single();
    ...
    return {
      unified,
      primary_name: data.primary_name as string | null,

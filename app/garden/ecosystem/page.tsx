...
function StrainCard({ strain: s, onTap }: { strain: Strain; onTap: () => void }) {
  ...
  return (
    ...
    >
      ...
      {hasDesc && (
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.78)", lineHeight: 1.4, marginBottom: hasEffects ? 8 : 0, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
          {s.description}
        </div>
      )}
      ...
      {hasEffects && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
          {s.effects.slice(0, 4).map((e) => (
            <span key={e} style={{ padding: "2px 6px", borderRadius: 6, fontSize: 12, background: "rgba(255,255,255,0.07)", color: "rgba(255,255,255,0.6)" }}>
              {EFFECT_ICONS[e.toLowerCase()] || "🌿"} {cap(e)}
            </span>
          ))}
          {s.effects.length > 4 && <span style={{ padding: "2px 6px", fontSize: 12, color: "rgba(255,255,255,0.65)" }}>+{s.effects.length - 4}</span>}
        </div>
      )}
    </div>
  );
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(10px)", display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 500, maxHeight: "88vh", overflowY: "auto", borderRadius: "24px 24px 0 0", background: "linear-gradient(180deg, #1a2420, #111816)", border: "1px solid rgba(255,255,255,0.1)", padding: 24 }}>
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 8 }}>
          <button onClick={onClose} style={{ background: "rgba(255,255,255,0.08)", border: "none", borderRadius: "50%", width: 32, height: 32, cursor: "pointer", color: "rgba(255,255,255,0.78)", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
        </div>
        ...
        {s.flavors && s.flavors.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.68)", marginBottom: 8, letterSpacing: "0.1em", textTransform: "uppercase" }}>Terpenes</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #111816, #0d120f)", color: "#fff" }}>
      <div style={{ padding: "16px 16px 96px", maxWidth: 600, margin: "0 auto" }}>
        ...
        <div style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, display: "flex", alignItems: "center", padding: "10px 16px", marginBottom: 12 }}>
          ...
          {searchInput && <button onClick={() => { setSearchInput(""); setSearch(""); }} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.65)", cursor: "pointer", fontSize: 16, padding: 4 }}>✕</button>}
        </div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
          {filters.map(({ label, value, emoji }) => {
            ...
            return (
              <button key={label} onClick={() => setTypeFilter(value)} style={{ padding: "6px 12px", borderRadius: 10, cursor: "pointer", background: active ? tc.bg : "rgba(255,255,255,0.03)", border: `1px solid ${active ? tc.border : "rgba(255,255,255,0.08)"}`, display: "flex", alignItems: "center", gap: 4, color: active ? tc.text : "rgba(255,255,255,0.75)", fontSize: 12, fontWeight: 600 }}>
                {emoji} {label}
              </button>
            );
          })}
          <button onClick={() => setSort(sort === "popular" ? "name" : "popular")} style={{ marginLeft: "auto", padding: "6px 12px", borderRadius: 10, cursor: "pointer", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.75)", fontSize: 13, fontWeight: 600 }}>
        ...
        <div ref={scrollRef} onScroll={handleScroll} style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: "calc(100vh - 260px)", overflowY: "auto", paddingRight: 4 }}>
          {loading ? (
            ...
            <>
              ...
              {loadingMore && <div style={{ textAlign: "center", padding: 16, color: "rgba(255,255,255,0.65)", fontSize: 13 }}>Loading more…</div>}
              {page >= totalPages && strains.length > 0 && <div style={{ textAlign: "center", color: "rgba(255,255,255,0.2)", fontSize: 13, padding: 16 }}>That's all {total.toLocaleString()} strains</div>}
            </>
          )}
        </div>
      </div>

  ...
  async function handleAnalyzePlant() {
    ...
    try {
      ...
      try {
        ...
        console.log("STEP 4: JUDGE RESULT RECEIVED", { best, candidates, askForBetterPics, guidance, description }, localScanResult);
      } catch (judgeErr) {
        console.warn("Judge path failed, falling back to orchestrator:", judgeErr);
        const orchestrated = await orchestrateScan(images);
        localScanResult = orchestrated.normalizedScanResult;
        console.log("STEP 4: RESULT RECEIVED (orchestrator)", localScanResult);
      }
      ...
      if (scanResult.status === "success" || scanResult.status === "partial") {
        ...
        }).catch(() => setHistorySaveNotice("Could not save to history (scan succeeded)."));
      }
    ...
    <>
      ...
      <div className="mx-auto w-full max-w-[720px] px-4 pb-24 md:pb-16 space-y-6">
        ...
        <section className="space-y-6 w-full max-w-[720px] mx-auto px-4">
          ...
          {historySaveNotice && (
            <p className="text-sm text-yellow-200/80">{historySaveNotice}</p>
          )}

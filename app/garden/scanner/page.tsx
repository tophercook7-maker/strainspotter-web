...
import { getStrainDatabase, getLimitedStrainsMode } from "@/lib/scanner/dbLoader";
...
export default function ScannerPage() {
  ...
  const [processing, setProcessing] = useState(false); // Backend process in progress (result_payload path)
  const [backendPhase, setBackendPhase] = useState<null | "uploading" | "processing" | "finalizing">(null); // Backend-first intermediate status
  const [backendFallbackNotice, setBackendFallbackNotice] = useState<string | null>(null); // Shown when backend-first fails and we fall back
  const [historySaveNotice, setHistorySaveNotice] = useState<string | null>(null); // Shown when saveScanHistory fails
  const [scanError, setScanError] = useState<{ reason: string } | null>(null); // Phase 4.0.1 — Non-fatal scan warnings
  const [imagePreviews, setImagePreviews] = useState<Array<{ url: string; base64: string; angleLabel: string; dataUrl: string }>>([]); // Phase 4.0.2 — Previews with base64 and angles
  ...
  useEffect(() => {
    ...
    setHistorySaveNotice(null);
  }, [images]);

  // Limited strains mode: load DB once and expose flag for banner (single warning in dbLoader)
  useEffect(() => {
    let cancelled = false;
    getStrainDatabase()
      .then(() => {
        if (!cancelled) setLimitedStrainsMode(getLimitedStrainsMode());
      })
      .catch(() => {
        if (!cancelled) setLimitedStrainsMode(true);
      });
    return () => {
      cancelled = true;
    };
    ...
    const convertImagesToPreviews = async () => {
      ...
      const previews = await Promise.all(
        images.map(async (img) => {
          ...
          return { url, base64, angleLabel, dataUrl };
        })
      );
  ...
  async function handleAnalyzePlant() {
    ...
    if (validation.warning && process.env.NODE_ENV === "development") {
      console.log("[StrainSpotter] Image count note:", validation.warning);
    }

    if (process.env.NODE_ENV === "development") {
      console.log("[StrainSpotter] Scan start", { imageCount: images.length });
    }
    ...
    try {
      // Primary image as data URL — read once, reuse for upload and judge/saveScanHistory fallback
      ...
      const backendEnabled = base.length > 0;

      // ——— Backend-first (single source of truth) ———
      if (backendEnabled) {
        try {
          setBackendPhase("uploading");
          const contentType = imageDataUrl.match(/^data:([^;]+);base64,/)?.[1] ?? "image/jpeg";
          const base64 = imageDataUrl.includes(",") ? imageDataUrl.split(",")[1] : imageDataUrl;
          const uploadResp = await fetch(`${base}/api/uploads`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ filename: "scan.jpg", contentType, base64 }),
          });
          const uploadJson = await uploadResp.json().catch(() => null);
          const scanId = uploadResp.ok && uploadJson?.id ? String(uploadJson.id) : null;
          if (!scanId) throw new Error("Upload did not return id");

          setBackendPhase("processing");
          const url = `${base}/api/scans/${scanId}/process`;
          const resp = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" } });
          const json = await resp.json().catch(() => null);
          if (!resp.ok) throw new Error("Process failed");

          let payload = json?.result_payload ?? json?.resultPayload ?? null;

          // If process didn't return v1 payload, poll our DB until status=done and result_payload v1 (max ~20s)
          if (!isScanResultPayloadV1(payload)) {
            ...
            for (let attempt = 0; attempt < maxAttempts; attempt++) {
              ...
              const pollRes = await fetch(`/api/scans/${scanId}`);
              const pollJson = await pollRes.json().catch(() => null);
              const status = pollJson?.status;
              const resultPayload = pollJson?.result_payload ?? null;
              if (status === "failed") {
                throw new Error("Backend scan failed");
              }
              if (status === "done" && isScanResultPayloadV1(resultPayload)) {
          ...
          setScanError(null);
          const vm = resultPayloadToFullScanResult(payload);
          const full: FullScanResult = { result: vm as unknown as FullScanResult["result"], analysis: undefined };
          setScanResult(full as any);
          setResult(vm as unknown as ScannerViewModel);
          setSynthesis(null);
          ...
          setBackendPhase(null);
          setBackendFallbackNotice("Backend failed — using local scan…");
          if (process.env.NODE_ENV === "development") {
            console.warn("[StrainSpotter] Backend scan failed, using local:", backendErr);
          }
          // Fall through to fallback
        }
      }

      // ——— Fallback: local judge/orchestrator when backend is unavailable ———
      let localScanResult: import("@/lib/scanner/types").ScanResult;
      try {
        const result = await runJudge(imageDataUrl);
        if (!result.ok) {
        ...
        localScanResult = judgeResultToScanResult(result, images.length);
      } catch (judgeErr) {
        if (process.env.NODE_ENV === "development") {
          console.warn("[StrainSpotter] Judge failed, using orchestrator:", judgeErr);
        }
        const orchestrated = await orchestrateScan(images);
        localScanResult = orchestrated.normalizedScanResult;
      }
      ...
      if (scanResult.status === "success" || scanResult.status === "partial") {
        ...
        saveScanHistory({
          ...
          model_version: (scanResult as any).meta?.model_version ?? null,
          matched_strain_slug: (scanResult.result as any)?.primaryMatch?.strain?.slug ?? (scanResult.result as any)?.namingInfo?.strainSlug ?? null,
        })
          .then(() => {
            if (process.env.NODE_ENV === "development") {
              console.log("[StrainSpotter] Scan success (local) — saved to Log Book");
            }
          })
          .catch((err) => {
            setHistorySaveNotice("Scan completed but could not save to Log Book. Results are still visible.");
            if (process.env.NODE_ENV === "development") {
              console.warn("[StrainSpotter] History save failed:", err);
            }
          });
      } else if (process.env.NODE_ENV === "development") {
        const status = (scanResult as { status?: string }).status;
        console.log("[StrainSpotter] Scan completed (local) — status:", status, "— not saved to history");
      }
      ...
      if (process.env.NODE_ENV === "development") {
        console.warn("[StrainSpotter] Scan failed:", errorMessage, "— user sees partial result");
      }
      setIsScanning(false);
    } finally {
      scanLockRef.current = false;
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLButtonElement>) {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      if (images.length > 0 && !isScanning && !processing && !scanLockRef.current) {
        handleAnalyzePlant();
      }
    }
  }
  ...
  return (
    <>
      ...
      <div className="w-full pt-6 pb-32 sm:pb-24 space-y-6">
        ...
        {limitedStrainsMode && (
          <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-4 text-sm text-amber-200 space-y-3 shadow-lg shadow-black/20">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                className="rounded-xl border border-white/15 bg-white/[0.06] px-3 py-2 text-sm font-semibold hover:bg-white/[0.1] hover:border-white/20 transition-colors"
              ...
              <button
                type="button"
                className="rounded-xl border border-white/15 bg-white/[0.03] px-3 py-2 text-sm hover:bg-white/[0.06] transition-colors"
        ...
        <div className="rounded-xl border border-white/15 bg-white/[0.06] backdrop-blur-xl p-5 sm:p-6 space-y-4 shadow-lg shadow-black/20">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-4">
                      {imagePreviews.map((preview, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className="flex flex-col rounded-xl border border-white/20 overflow-hidden group"
                          >
                            ...
                            <div className="relative aspect-square rounded-t-xl overflow-hidden bg-black/20">
                              <img
                                src={preview.url}
                                alt={`scan-${idx + 1}`}
                                className="w-full h-full object-contain max-h-[200px] sm:max-h-[260px]"
                              />
                            ...
                            {/* Phase 5.2.2 — Image label and controls (always visible, no overflow) */}
                            <div className="flex items-center justify-between gap-2 px-2 py-2 min-h-[44px] bg-black/20 border-t border-white/10">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                          {guidance.slots
                            ...
                            .map((slot) => (
                              <div
                                key={slot.slotNumber}
                                className="rounded-xl border border-white/15 bg-white/[0.06] p-3 text-center"
                              >
        ...
        <div className="rounded-xl border border-white/15 bg-white/[0.06] backdrop-blur-xl p-5 sm:p-6 sticky bottom-24 sm:bottom-6 md:static z-10 bg-black/95 md:bg-transparent backdrop-blur-sm md:backdrop-blur-none shadow-lg shadow-black/20">
          ...
          <div className="w-full flex justify-center">
            <ClearButton
              ...
              disabled={isScanning || processing || images.length === 0}
              startIcon={
                (isScanning || processing) ? <CircularProgress size={18} color="inherit" /> : undefined
              }
              aria-label="Run scan"
              aria-busy={isScanning || processing}
              sx={{
                minWidth: 200,
                maxWidth: '28rem',
                height: 52,
        ...
        <section className="space-y-6 w-full">
          ...
          {backendPhase !== null && (
            <p className="text-sm text-white/70">
              {backendPhase === "uploading" && "Uploading scan…"}
              {backendPhase === "processing" && "Analyzing…"}
              {backendPhase === "finalizing" && "Finalizing results…"}
            </p>
          )}
          {backendFallbackNotice && (
            <p className="text-sm text-yellow-200/80">{backendFallbackNotice}</p>
          )}
          {historySaveNotice && (
            <div className="rounded-xl border border-yellow-500/40 bg-yellow-500/10 p-3 text-sm text-yellow-200 shadow-lg shadow-black/20">
              <span className="font-medium">Note:</span> {historySaveNotice}
            </div>
          )}
          {/* FAILURE MESSAGING SOFTENED — User-facing warnings (non-fatal, non-blocking) */}
          {scanError?.reason === "images-too-similar" && (
            <div className="mt-4 max-w-md mx-auto rounded-xl bg-yellow-900/30 border border-yellow-700/50 p-4 text-sm shadow-lg shadow-black/20">
          ...
          {scanResult && 'status' in scanResult && scanResult.status === "partial" && (
            <div className="mt-4 rounded-xl border border-white/15 bg-white/[0.06] p-4 text-sm text-white/80 shadow-lg shadow-black/20">
          ...
          {analysis && (
            <WikiStyleResultPanel
              result={analysis}
              flags={flags}
              imageDataUrl={imagePreviews[0]?.dataUrl ?? null}
            />
          )}

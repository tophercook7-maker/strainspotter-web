...
function confidenceFromPayload(p: unknown): number | null {
  if (!p || typeof p !== "object") return null;
  const c = (p as { primary_match?: { confidence?: number } }).primary_match?.confidence;
...
async function getStrainNodes(): Promise<EcosystemStrainNode[]> {
  try {
    ...
    // Group by strain name from result_payload or fallback
    ...
    for (const scan of data) {
      ...
      const name = label === "Scan" ? "Unknown Strain" : label;
      ...
      const conf = confidenceFromPayload(scan.result_payload);
      if (conf != null) {
        stats.totalConfidence += conf;

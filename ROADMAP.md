...
- ⬜ **Re-prioritized next levers** (in order): (1) stronger embedding model — SigLIP / larger or fine-tuned CLIP / OpenAI image embeddings; (2) feed reference **images** (not just names) so GPT-4o can visually compare; (3) more + cleaner reference images per strain (**this is the Phase 2 data moat**). Then re-run the A/B. Move to **pgvector** once retrieval is worth scaling.

...
-- Policies: users only see and manage their own notes (idempotent)
DROP POLICY IF EXISTS "Grow Notes: select own" ON grow_notes;
CREATE POLICY "Grow Notes: select own"
  ON grow_notes FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Grow Notes: insert own" ON grow_notes;
CREATE POLICY "Grow Notes: insert own"
  ON grow_notes FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Grow Notes: update own" ON grow_notes;
CREATE POLICY "Grow Notes: update own"
  ON grow_notes FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Grow Notes: delete own" ON grow_notes;
